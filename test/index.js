...
describe('#foaas', function() {
  ...
  it('returns the version details from the API', function() {
    return foaas.getVersion({type: 'json'}).then(function(data) {
      data.should.equal('0.1.4');
    });
  });

  it('says fuck off to Tom, from Chris', function() {
    return foaas.off('Tom', 'Chris', {type: 'json'}).then(function(data) {
  ...
  it('says fuck this, from Chris', function() {
    return foaas.off('Chris', {type: 'json'}).then(function(data) {
  ...
  it('gets linus', function() {
    return foaas.linus('Tom', 'Chris', {type: 'json'}).then(function(data) {
  ...
  it('gets kingly', function() {
    return foaas.king('Tom', 'Chris', {type: 'json'}).then(function(data) {
  ...
  it('finds it cool', function() {
    return foaas.cool('Tom', {type: 'json'}).then(function(data) {
      data.should.equal('Cool story, Bro - Tom');
    });
  });

  it('finds itself glancing over the field', function() {
    return foaas.field('Tom', 'Chris', 'Anon', {type: 'json'}).then(function(data) {
  ...
  it('says something about a bus', function() {
    return foaas.diabetes('Tom', 'Chris', {type: 'json'}).then(function(data) {
  ...
  it('wishes you season\'s greetings', function() {
    return foaas.xmas('Tom', 'Chris', {type: 'json'}).then(function(data) {
      data.should.equal('Merry Fucking Christmas, Tom. - Chris');
    });
  });

  it('says this is awesome', function() {
    return foaas.awesome('Macklemore', {type: 'json'}).then(function(data) {
      data.should.equal('This is Fucking Awesome. - Macklemore');
    });
  });

  it('SHOUTS this is awesome', function() {
    return foaas.awesome('Macklemore', {type: 'json', shoutcast: true}).then(function(data) {
      data.should.equal('THIS IS FUCKING AWESOME. - MACKLEMORE');
    });
  });

  it('says this is awesome in Spanish', function() {
    return foaas.awesome('Macklemore', {type: 'json', i18n: 'es'}).then(function(data) {
      data.should.equal('Esto es jodidamente increíble. - Macklemore');
    });
  });

  it('SHOUTS this is awesome in Spanish', function() {
    return foaas.awesome('Macklemore', {type: 'json', shoutcast: true, i18n: 'es'}).then(function(data) {
      data.should.equal('Esto es jodidamente increíble. - Macklemore');
    });
  });
